...
# 1) Ensure module directory
$modulePath = "js/modules/services-dashboard-pricing.js"
$moduleDir  = Split-Path $modulePath
if (!(Test-Path $moduleDir)) { New-Item -ItemType Directory -Path $moduleDir | Out-Null }

# 2) Write full module file (overwrites if exists)
@'
/**
...
# 3) Inject script tag into services.html (idempotent)
$services = "services.html"
if (!(Test-Path $services)) {
  throw "services.html not found at repo root. Update the path to your Services dashboard file, then re-run."
}

# Read current file
$content = Get-Content $services -Raw

$scriptTag = '<script type="module" src="/js/modules/services-dashboard-pricing.js" defer></script>'
if ($content -notmatch [regex]::Escape($scriptTag)) {
  # Backup once
  $bak = "$services.bak"
  if (!(Test-Path $bak)) {
    Copy-Item $services $bak
  }
  ...
  if ($content -match '</body>') {
    ...
    $updated = $content + "`r`n$scriptTag`r`n"
  }
  $updated | Set-Content $services -Encoding UTF8
  Write-Host "Patched $services with NSM-20 script tag."
} else {
  Write-Host "Script tag already present in $services. No changes made."
}

Write-Host "NSM-20 module created and services.html patched successfully."
